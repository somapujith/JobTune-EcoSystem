...
export default function PortfolioBuilder() {
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] overflow-hidden w-full">
      {/* Sidebar */}
      <div className="w-80 card rounded-r-2xl rounded-l-none border-l-0 flex flex-col hidden sm:flex shadow-none">
        <div className="p-8">
          <div className="flex items-center gap-3 mb-2">
            <span className="material-symbols-outlined text-indigo-600 text-2xl" style={{ fontVariationSettings: "'FILL' 0" }}>web</span>
            <h2 className="text-xl font-extrabold text-on-surface font-headline">Portfolio Blocks</h2>
          </div>
        ...
        <div className="px-8 pb-8 space-y-4 overflow-y-auto flex-grow">
          <button className="w-full text-left p-4 card hover:bg-white/40 rounded-2xl transition-all duration-200 flex items-center gap-4 group">
      ...
      <div className="flex-grow overflow-y-auto p-4 sm:p-8">
        <div className="page-container card min-h-[900px] rounded-2xl overflow-hidden">
          {sections.map(sec => (
            <div key={sec.id} className={`group relative hover:shadow-[0px_10px_30px_rgba(0,78,159,0.15)] transition-all duration-300 ${sec.bg || 'card border-none'}`}>
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity card text-on-surface text-sm px-4 py-2 rounded-2xl shadow-lg font-bold cursor-pointer border flex items-center gap-2">
              ...
              {sec.type === 'hero' && (
                <div className="py-32 px-16 text-center">
                  <h1 className="text-6xl font-extrabold tracking-tight mb-8 font-headline">{sec.title}</h1>
                  <p className="text-xl text-on-surface-variant font-medium max-w-2xl mx-auto leading-relaxed">{sec.subtitle}</p>
                </div>
              )}
              {sec.type === 'about' && (
                <div className="py-20 px-16">
                  <h3 className="text-3xl font-extrabold mb-8 font-headline">About Me</h3>
