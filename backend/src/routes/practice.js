...
(async () => {
  try {
    ...
    console.error('Practice table migration error:', err.message);
  }
})();
...
router.get('/problems', authenticateToken, requirePlan(1), async (req, res) => {
  try {
...
router.get('/problems/:id', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const problem = FALLBACK_PROBLEMS.find(p => p.id === req.params.id);
    if (!problem) {
...
// POST /problems/:id/run — run code against test cases
router.post('/problems/:id/run', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const problem = FALLBACK_PROBLEMS.find(p => p.id === req.params.id);
    if (!problem) {
      return res.status(404).json({ error: 'Problem not found' });
...
Evaluate this code against the test cases.`;
    ...
    try {
      ...
      results = extractJSON(aiResponse);
    } catch {
      // Fallback results
      results = {
...
router.post('/problems/:id/submit', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const problem = FALLBACK_PROBLEMS.find(p => p.id === req.params.id);
    if (!problem) {
      return res.status(404).json({ error: 'Problem not found' });
...
Evaluate this solution thoroughly.`;
    ...
    try {
      ...
      evaluation = extractJSON(aiResponse);
    } catch {
      evaluation = {
        passed: false,
        results: problem.testCases.map((tc, i) => ({
    ...
    try {
      await pool.query(
        ...
        [req.user.id, problem.id, code, language, evaluation.passed || false, JSON.stringify(evaluation.results), evaluation.feedback]
      );
    } catch (dbErr) {
...
// POST /problems/:id/hint — get AI hint
router.post('/problems/:id/hint', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const problem = FALLBACK_PROBLEMS.find(p => p.id === req.params.id);
    if (!problem) {
      return res.status(404).json({ error: 'Problem not found' });
...
Please provide a Level ${level} hint for this problem.`;
    ...
    try {
      ...
      hintData = extractJSON(aiResponse);
    } catch {
      hintData = {
...
router.get('/assessments', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const userId = req.user.id;
...
router.get('/assessments/:id', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const assessment = FALLBACK_ASSESSMENTS.find(a => a.id === req.params.id);
    if (!assessment) {
...
router.post('/assessments/:id/submit', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const assessment = FALLBACK_ASSESSMENTS.find(a => a.id === req.params.id);
    if (!assessment) {
      return res.status(404).json({ error: 'Assessment not found' });
    ...
    try {
      await pool.query(
        ...
        [req.user.id, assessment.id, JSON.stringify(answers), score, total, timeTaken || null, JSON.stringify(results)]
      );
    } catch (dbErr) {
...
// GET /stats — user practice stats
router.get('/stats', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    const userId = req.user.id;
...
// POST /problems/:id/bookmark — toggle bookmark
router.post('/problems/:id/bookmark', authenticateToken, requirePlan(1), async (req, res) => {
  try {
    ...
    if (existing.rows.length > 0) {
      await pool.query('DELETE FROM practice_bookmarks WHERE user_id = $1 AND problem_id = $2', [userId, problemId]);
      res.json({ bookmarked: false });
    } else {
      await pool.query('INSERT INTO practice_bookmarks (user_id, problem_id) VALUES ($1, $2)', [userId, problemId]);
