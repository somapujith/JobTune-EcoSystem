...
router.post('/faculty/assignments', authenticateToken, async (req, res) => {
  try {
    ...
    if (!title || !dueDate || !courseId) {
      return res.status(400).json({ error: 'Title, due date, and course are required' });
    }

    const course = DEMO_COURSES.find(c => c.id === parseInt(courseId, 10));
    const assignment = {
      id: Date.now(),
      title,
...
router.post('/recruiter/shortlist', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ pipeline, message: `Student moved to ${targetStatus}` });
  } catch (err) {
