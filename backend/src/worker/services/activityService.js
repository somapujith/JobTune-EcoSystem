...
/**
 * Worker port of backend/src/services/activityService.js (ported from git HEAD 38d8130a).
 *
 * Dependency injection only: `createActivityService({ db })` instead of a singleton bound to a
 * module-level pool. Method names, SQL text, signatures and return shapes are unchanged.
 * Methods are closures, so they are safe to destructure.
 *
 * Preserved on purpose (do NOT "fix" during the port, ADR 4.3):
 *   - getWeeklyActivity computes the Monday of the current week with the RUNTIME's local time
 *     (getDay / setDate / setHours) and then serialises with toISOString (UTC). That is only
 *     consistent when the runtime timezone is UTC. Workers always run in UTC; Render runs in
 *     whatever TZ the box has (UTC by default, unverified here).
 *   - "date" values are handled both as Date objects and as strings, like the original.
 *   - getStats swallows errors from the learning_streaks / today queries and returns defaults;
 *     checkAndUnlockAchievements swallows per-achievement errors. The learning_streaks queries
 *     here select columns (current, longest, daily_goal), which is the shape routes/courses.js
 *     creates; learningPathService and the Express boot-time table bootstrap use another shape
 *     (subject, current_streak, longest_streak). Whichever CREATE TABLE ran first in production
 *     decides which of the two code paths works; the mismatch is preserved as-is and hidden by
 *     the swallowed errors. Which shape the live table has was NOT checked (no DB access).
 */
...
function createActivityService({ db }) {
  if (!db || typeof db.query !== 'function') throw new TypeError('createActivityService requires a db with query()');

  /**
   * Track a user activity. UPSERTs into daily_activity, incrementing count on conflict.
   */
  async function trackActivity(userId, toolName, activityType, metadata = {}) {
   ...
   */
  async function getHeatmapData(userId, months = 12) {
    const result = await db.query(
    ...
    const checks = [
      {
        key: 'first_login',
        check: async () => {
          const r = await db.query('SELECT 1 FROM daily_activity WHERE user_id = $1 LIMIT 1', [userId]);
      ...
      },
      {
        key: 'first_resume',
        check: async () => {
          const r = await db.query('SELECT 1 FROM resumes WHERE user_id = $1 LIMIT 1', [userId]);
          return r.rows.length > 0;
        }
      },
      {
        ...
        check: async () => {
          const r = await db.query('SELECT 1 FROM skill_assessments WHERE user_id = $1 LIMIT 1', [userId]);
          return r.rows.length > 0;
        }
      },
      {
        key: 'first_interview',
        check: async () => {
          const r = await db.query('SELECT 1 FROM mock_interviews WHERE user_id = $1 LIMIT 1', [userId]);
          return r.rows.length > 0;
        }
      },
      {
      ...
      },
      {
        ...
        check: async () => {
          const [resume, assessment, linkedin] = await Promise.all([
            db.query('SELECT 1 FROM resumes WHERE user_id = $1 LIMIT 1', [userId]),
            db.query('SELECT 1 FROM skill_assessments WHERE user_id = $1 LIMIT 1', [userId]),
            db.query('SELECT 1 FROM linkedin_analyses WHERE user_id = $1 LIMIT 1', [userId])
          ]);
