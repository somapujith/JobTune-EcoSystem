 ...
 *
 * Worker port (leaf1 slice) of backend/src/services/guides/jobGuideGenerator.js.
 * Logic, prompts, SQL, error messages and return shapes are unchanged. Differences:
 *   - the database is the injected request-scoped `db` (not the shared pool);
 *   - the LLM is reached through the injected `services.aiClient.callAI` (infra slice), looked up
 *     at call time inside the same try block that already turned any callAI failure into the
 *     template fallback. Consequence: if aiClient is not registered/wired, guides silently come
 *     back as the template guide (Express did the same for a failing provider).
 *
 *   const { jobGuideGenerator } = getServices(c);   // registry key: jobGuideGenerator
 */
...
/**
 * @param {{ db: { query: Function }, services: { aiClient: { callAI: Function } } }} deps
 */
function createJobGuideGenerator({ db, services }) {
  // ─── LLM guide builder ──────────────────────────────────────────────────────
  ...
  async function buildLLMGuide({ role, company, jobDescription }) {
    ...
    try {
      const { ok, data } = await services.aiClient.callAI({ systemPrompt, userPrompt, maxTokens: 1024, temperature: 0.5 });
  ...
  return { generateJobGuide, getJobGuide, buildFallbackGuide };
}

module.exports = { createJobGuideGenerator, buildFallbackGuide };
