...
/**
 * Worker port of backend/src/services/recommendationEngine.js (git HEAD 38d8130a, identical
 * in the working tree).   (T2.8, ADR-001 section 7)
 *
 * The scoring rules, weights, plan names and return shape are copied verbatim; nothing about
 * the recommendation logic changed. The only difference is dependency injection: the original
 * `module.exports = new RecommendationEngine()` singleton imported the planService singleton,
 * here `createRecommendationEngine({ services })` reads `services.planService` when
 * recommendPlan runs (lazy, so a test override of planService is honoured and there is no
 * construction-order dependency). A `planService` may also be passed directly for unit tests.
 *
 * Public API (same as the instance API of the original): recommendPlan(careerGoal,
 * experienceLevel, painPoints, fieldOfInterest) -> { recommendedPlan, allPlans, scores }.
 */

/**
 * @param {{ services?: { planService: { getAllPlans: Function } }, planService?: { getAllPlans: Function } }} deps
 */
function createRecommendationEngine({ services, planService } = {}) {
  if (!planService && !services) {
    throw new TypeError('createRecommendationEngine requires { services } or { planService }');
  }
  const getPlanService = () => planService || services.planService;

  async function recommendPlan(careerGoal, experienceLevel, painPoints, fieldOfInterest) {
    ...
    const allPlansData = await getPlanService().getAllPlans();
