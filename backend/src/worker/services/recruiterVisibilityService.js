...
/**
 * Worker port (leaf1 slice) of backend/src/services/recruiterVisibilityService.js.
 * Pure, stateless logic with no ambient dependencies (no db, no network, no config), so the body
 * below is copied verbatim from the Express module. Only the export changed: a factory with the
 * same public API for the service container (registry key: recruiterVisibilityService). The
 * functions are also exported directly because they hold no per-request state.
 */
...
function createRecruiterVisibilityService() {
  return { computeVisibility, scoreResume, scoreLinkedIn, scoreGitHub, scoreKeywords, RECRUITER_VOCABULARY };
}
