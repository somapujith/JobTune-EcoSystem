 ...
 *
 * Worker port (leaf1 slice) of backend/src/services/benchmarks/scorerBenchmark.js.
 * The fixture data and the scorer/metric functions below are copied verbatim. The one change:
 * runBenchmark persists through the injected db instead of the shared connection pool.
 *
 *   const { scorerBenchmark } = getServices(c);   // registry key: scorerBenchmark
 *   await scorerBenchmark.runBenchmark({ scorerName, datasetName });
 */
...
/**
 * @param {{ db: { query: Function } }} deps  request-scoped db (pg-style query surface)
 */
function createScorerBenchmark({ db }) {
  /**
  ...
  return { runBenchmark, computeMetrics, FIXTURE_DATASET };
}

module.exports = { createScorerBenchmark, computeMetrics, FIXTURE_DATASET };
