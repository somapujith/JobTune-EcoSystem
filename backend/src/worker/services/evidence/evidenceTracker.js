...
/**
 * Worker port of backend/src/services/evidence/evidenceTracker.js (ported from git HEAD 38d8130a).
 * Extracts, hashes, embeds, and tracks resume bullet reuse.
 *
 * Dependency injection: `createEvidenceTracker({ db, services })` instead of module-scope `pool`
 * and a direct import of the embeddings util. The public API is identical to the original module's
 * exports: extractBullets, saveBullet, logUsage, getReuseReport, hashBullet, extractSkillsFromText,
 * calculateDiversityScore.
 *
 * `embedText` comes from the infra slice's injected `services.embeddings` (same signature as
 * utils/embeddings.embedText). It is resolved lazily, at call time, so building this service never
 * builds the embeddings service.
 *
 * `crypto` is Node's, provided under the nodejs_compat flag (ADR spike S10 covers createHash).
 */
...
/**
 * @param {{ db: { query: Function }, services?: { embeddings?: { embedText: Function } } }} deps
 */
function createEvidenceTracker({ db, services }) {
  if (!db || typeof db.query !== 'function') throw new TypeError('createEvidenceTracker requires a db with query()');
  ...
  async function saveBullet(userId, bullet, sourceSection = 'experience') {
    const embedding = await services.embeddings.embedText(bullet.text);
...
module.exports = { createEvidenceTracker };
