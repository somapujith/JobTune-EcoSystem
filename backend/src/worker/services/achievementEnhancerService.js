 ...
 * (Worker port of backend/src/services/achievementEnhancerService.js; exposed as
 * `getServices(c).achievementEnhancerService`.)
 *
 ...
 *
 * Porting notes: the Express module required the two v2 analyzers directly. Those are pure modules owned by
 * the infra slice; this port takes them from the injected service container instead
 * (`services.actionVerbAnalyzer` with `.analyze` and `.STRONG_VERBS`, `services.metricsAnalyzer` with
 * `.analyze`, i.e. the original modules' exports). They are resolved lazily at call time, so nothing is
 * looked up at module scope. Exports and logic are otherwise verbatim.
 */
...
/**
 * @param {{ services: { actionVerbAnalyzer: object, metricsAnalyzer: object } }} deps
 */
function createAchievementEnhancerService({ services }) {
  /**
  ...
  function detectContext(achievement, roleHint = '') {
    ...
    const verbAnalysis = services.actionVerbAnalyzer.analyze(achievement || '');
  ...
  function extractImpact(achievement) {
    ...
    const metricsAnalysis = services.metricsAnalyzer.analyze(text);

    const tokens = tokenize(text);
    const leadingVerb = tokens.length ? tokens[0].toLowerCase() : null;
    const hasStrongVerb = leadingVerb
      ? services.actionVerbAnalyzer.STRONG_VERBS.includes(leadingVerb)
      : false;
...
module.exports = { createAchievementEnhancerService, DOMAIN_KEYWORDS, IMPACT_PHRASES };
