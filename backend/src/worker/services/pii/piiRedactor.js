...
/**
 * Worker port of backend/src/services/pii/piiRedactor.js (ported from git HEAD 38d8130a).
 * Pure text logic, no database: ported verbatim. The only change is the factory wrapper
 * (`createPiiRedactor()` returns the same `{ redact, restore }` the original module exported).
 *
 * Preserved on purpose (do NOT "fix" during the port, ADR 4.3):
 *   - restore() passes the ORIGINAL value to String.replace as the replacement string, so a
 *     stored value containing `$&`, `$1`, `$$` etc. is interpreted as a replacement pattern.
 *   - the name lists are US-centric and case-sensitive; ADDRESS is case-insensitive.
 */
...
module.exports = { createPiiRedactor };
