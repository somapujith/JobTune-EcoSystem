...
/**
 * Worker port of backend/src/services/linkedinOptimizerService.js (ported from git HEAD 38d8130a).
 * (ADR-001 Phase 2/3, slice "large2")
 *
 * Same exported API as the original module (analyzeLinkedInProfile, normalizeProfileInput,
 * fetchLinkedInPublicData, deriveProfileSignals, keywordIntelligence, generateHeadline,
 * generateAbout, generateExperienceDescription); scoring, prompts, fallbacks and response
 * shapes are the original text. Registry key: `linkedinOptimizerService`
 * (see services/registry/large2.js).
 *
 * What changed (only what the platform forces, plus dependency injection):
 *   - `createLinkedinOptimizerService({ config, services })` instead of a module of free
 *     functions. The pure helpers stay at module scope (no state, no globals).
 *   - callAI / extractJSON come from the injected `services.aiClient` (infra-owned contract:
 *     the same exports as utils/aiClient.js). They are resolved at CALL time, not at
 *     construction time, so test overrides and lazy container wiring work. A missing
 *     aiClient service throws (loud 500 at the route) rather than silently degrading to the
 *     rule-based fallback; on Express the module import could not be missing, so this only
 *     changes behaviour in a mis-wired deployment.
 *   - the model override reads config.vars.LM_STUDIO_MODEL_LINKEDIN / LM_STUDIO_MODEL_GITHUB /
 *     LM_STUDIO_MODEL (the same variables the original read from the process environment)
 *     at call time.
 *   - fetchLinkedInPublicData uses the platform global fetch + AbortSignal.timeout (already
 *     what the original used). Egress differs on Workers (Cloudflare IPs instead of Render's),
 *     so LinkedIn's anti-bot response (HTTP 999 / authwall) may differ; that path already
 *     degrades to "provided data only" and is unverified here.
 */
...
/**
 * @param {{ config?: { vars?: Record<string, string|undefined> }, services: { aiClient: object } }} deps
 */
function createLinkedinOptimizerService({ config, services }) {
  const vars = (config && config.vars) || {};

  // Resolved per call so an override / late registration is honoured (see header note).
  function aiClient() {
    const client = services && services.aiClient;
    if (!client || typeof client.callAI !== 'function' || typeof client.extractJSON !== 'function') {
      throw new Error('aiClient service is not registered');
    }
    return client;
  }
  const callAI = (options) => aiClient().callAI(options);
  const extractJSON = (text) => aiClient().extractJSON(text);

  // NOTE: the functions below are the original text, left at their original indentation on purpose:
  // the LLM prompts are multi-line template literals and re-indenting would change the prompt bytes.
...
Generate truthful, specific optimization guidance.`;
  ...
  const aiResult = await callAI({
    ...
    maxTokens: 1500,
    temperature: 0.35,
    model: vars.LM_STUDIO_MODEL_LINKEDIN || vars.LM_STUDIO_MODEL,
    structuredJson: true,
  });
...
Target positions: ${Array.isArray(targetRoles) ? targetRoles.join(', ') : targetRoles || 'N/A'}`;
  ...
  const result = await callAI({
    ...
    maxTokens: 500,
    temperature: 0.7,
    model: vars.LM_STUDIO_MODEL_GITHUB || vars.LM_STUDIO_MODEL,
    structuredJson: true,
  });
...
module.exports = { createLinkedinOptimizerService };
