 ...
 * (Worker port of backend/src/services/scoring/strategies/jobFit.js; exposed as `getServices(c).jobFit`.)
 *
 ...
 *
 * Porting notes:
 *   - The Express module imported `findOccupation`/`getDomain` (taxonomy/onetLoader) and
 *     `callAI`/`extractJSON` (utils/aiClient) at module scope, and read the fit model name from the
 *     process environment. Here `createJobFitScorer({ config, services })` reaches them lazily through the
 *     injected container (`services.onetLoader`, `services.aiClient`, both infra-owned) and reads
 *     `config.vars.LM_STUDIO_MODEL_FIT`. The public surface is still `{ score }`.
 *   - Behaviour is otherwise verbatim: same weights, prompts, clamping and rule-based fallback.
 *   - The aiClient is resolved at the start of every call, OUTSIDE the try/catch that turns any LLM failure
 *     into "fall back to rules": a missing/mis-wired aiClient service must fail loudly (500) rather than
 *     silently degrade every score to the rule-based path. Provider failures still fall back exactly as before.
 */
 ...
 * @param {{ findOccupation: Function, getDomain: Function }} onetLoader  services.onetLoader
 * @param {string} resumeText
 * @param {string} jobDescription
 * @returns {number} 0-100
 */
function ruleDomainScore(onetLoader, resumeText, jobDescription) {
  const resumeOcc = onetLoader.findOccupation(resumeText.slice(0, 200));
  const jobOcc = onetLoader.findOccupation(jobDescription.slice(0, 200));

  const resumeDomain = onetLoader.getDomain(resumeOcc);
  const jobDomain = onetLoader.getDomain(jobOcc);
...
/**
 * @param {{ config: { vars?: object }, services: object }} deps
 *   config    the injected frozen config (reads config.vars.LM_STUDIO_MODEL_FIT)
 *   services  the lazy service container (services.aiClient, services.onetLoader)
 * @returns {{ score: Function }}
 */
function createJobFitScorer({ config, services }) {
  // ─── LLM scoring ───────────────────────────────────────────────────────────
  ...
  async function tryLLMScore(resumeText, jobDescription) {
    // resolved and checked outside the try on purpose (see header note): only provider failures may fall back
    const aiClient = services.aiClient;
    if (!aiClient) throw new Error('jobFit scorer: services.aiClient is not available');
    try {
...
${(jobDescription || '').slice(0, 300)}`;
      ...
      const result = await aiClient.callAI({
        ...
        maxTokens: 500,
        temperature: 0.2,
        model: ((config && config.vars) || {}).LM_STUDIO_MODEL_FIT || 'qwen2.5-7b'
      });

      if (!result.ok || !result.data) return null;

      const parsed = aiClient.extractJSON(result.data);
  ...
  async function score({ resumeText, jobDescription }) {
    ...
    if (llmResult) {
      ...
      domainScore = ruleDomainScore(services.onetLoader, resume, jd);
...
module.exports = { createJobFitScorer };
