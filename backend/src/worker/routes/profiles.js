...
/**
 * Profiles Routes (GitHub optimizer, LinkedIn optimizer, job match)   mount: /api/profiles
 * Worker port of backend/src/routes/profiles.js (ported from git HEAD 38d8130a). ADR-001 wave 3F, slice "large2".
 *
 * requirePlan(2) on github/analyze, github/generate-repo-readme, github/optimize-bio, github/save,
 * linkedin/analyze, linkedin/generate-headline, linkedin/generate-about, linkedin/generate-experience.
 * authenticateToken only on github/history, linkedin/history, linkedin/history/:id, jobmatch.
 *
 * Port notes (details in docs/migration/wave/large2.md):
 *   - handlers, scoring engine, prompts, fallbacks, status codes and response bodies are the Express text;
 *     only the request/response plumbing changed (c.get('user'), getBody(c), getQuery(c), c.req.param, c.json, getDb(c)).
 *   - callAI / extractJSON come from the injected aiClient service; the LinkedIn helpers come from the injected
 *     linkedinOptimizerService / linkedinAnalysisStore services (ported in ../services/).
 *   - the model override reads config.vars.LM_STUDIO_MODEL_GITHUB / LM_STUDIO_MODEL at request time.
 *   - Buffer.from(base64) for the profile README is replaced by decodeBase64Utf8 (same decoding rules, fuzz-tested
 *     against Buffer in tests/worker/large2/base64.test.js); no Node global is needed.
 *   - the outbound GitHub calls use the global fetch + AbortSignal.timeout exactly as before (host allowlist kept).
 *   - preserved quirks: /jobmatch is a synchronous handler with no try/catch, so a non-string jobDescription or
 *     userSkills, or a request with no JSON body, is a masked 500; /github/analyze lower-cases the username;
 *     /linkedin/history/:id passes the raw id to SQL (a non-numeric id is a Postgres error -> 500).
 */
 ...
 * (a loud failure at the route) instead of quietly degrading to the rule-based fallback. Not reachable on
 * Express, where the module import cannot be missing.
 */
function getAi(c) {
...
/** A named injected service (linkedinOptimizerService, linkedinAnalysisStore); throws loudly if not registered. */
function service(c, name) {
  const svc = getServices(c)[name];
  if (!svc) throw new Error(name + ' service is not registered');
  return svc;
}

/** Was: the LM_STUDIO_MODEL_GITHUB || LM_STUDIO_MODEL process-environment read (now config, at request time). */
function githubModel(c) {
  const vars = getConfig(c).vars;
  return vars.LM_STUDIO_MODEL_GITHUB || vars.LM_STUDIO_MODEL;
}

/**
 * Replacement for Buffer.from(content, 'base64').toString('utf8') (the GitHub contents API returns base64 with
 * a newline every 60 characters). Mirrors Node's lenient decoder: stops at the first '=', ignores characters
 * outside the base64 (and base64url) alphabet, drops a dangling sextet, keeps a leading BOM, and replaces
 * invalid UTF-8 with U+FFFD. Throws (caught by the caller, like a bad Buffer.from argument) for non-strings.
 */
function decodeBase64Utf8(value) {
  if (typeof value !== 'string') throw new TypeError('base64 content must be a string');
  let text = value;
  const pad = text.indexOf('=');
  if (pad !== -1) text = text.slice(0, pad);
  text = text.replace(/-/g, '+').replace(/_/g, '/').replace(/[^A-Za-z0-9+/]/g, '');
  if (text.length % 4 === 1) text = text.slice(0, -1);
      ...
      try {
        ...
        if (readmeRes.ok) {
          ...
          if (readmeJson.content) {
            existingReadmeContent = decodeBase64Utf8(readmeJson.content);
          }
        }
      } catch (e) {
...
async function getAIEnhancements(ai, model, ghData, scores, showcaseProjects, issues) {
  const repos = ghData.repos || [];
...
router.post('/github/analyze', authenticateToken, requirePlan(2), async (c) => {
  try {
    let { username } = getBody(c);
    if (!username) return c.json({ error: 'GitHub username or URL is required' }, 400);
    ...
    if (!username || !/^[a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38}$/i.test(username)) {
      return c.json({ error: 'Invalid GitHub username format.' }, 400);
    }

    const ghData = await fetchGitHubData(username);
    if (!ghData) {
      return c.json({ error: `GitHub user "${username}" not found or API unavailable.` }, 404);
    }

    const { scores, grade, issues } = scorePortfolio(ghData);
    const { showcaseProjects, hasPortfolio } = categorizeProjects(ghData.repos);
    const stage4 = await getAIEnhancements(getAi(c), githubModel(c), ghData, scores, showcaseProjects, issues);
...
router.post('/github/generate-repo-readme', authenticateToken, requirePlan(2), async (c) => {
  try {
    const { username, repoName, repoDescription, language, topics, stars } = getBody(c) || {};
...
`;
    ...
    return c.json({ error: 'Failed to generate README' }, 500);
  }
});

// POST /api/profiles/github/optimize-bio — rewrite a bio under 160 chars.
router.post('/github/optimize-bio', authenticateToken, requirePlan(2), async (c) => {
  try {
    const { currentBio, name, languages, targetRole } = getBody(c) || {};
...
Make it role-first and recruiter-friendly. Return the JSON now.`;
    ...
    return c.json({ error: 'Failed to optimize bio' }, 500);
  }
});

// POST /api/profiles/github/save — persist an analysis.
router.post('/github/save', authenticateToken, requirePlan(2), async (c) => {
  try {
    const { username, scores, grade, report, stage4 } = getBody(c) || {};
...
router.get('/github/history', authenticateToken, async (c) => {
  try {
    const userId = c.get('user').id;
    const result = await getDb(c).query(
...
router.post('/linkedin/analyze', authenticateToken, requirePlan(2), async (c) => {
  try {
    const linkedinOptimizerService = service(c, 'linkedinOptimizerService');
    const linkedinAnalysisStore = service(c, 'linkedinAnalysisStore');
    const report = await linkedinOptimizerService.analyzeLinkedInProfile(getBody(c) || {});

    try {
      const saved = await linkedinAnalysisStore.saveLinkedInAnalysis(c.get('user').id, report);
    ...
    return c.json({ error: 'LinkedIn analysis failed. Please try again.' }, 500);
  }
});

router.get('/linkedin/history', authenticateToken, async (c) => {
  try {
    const linkedinAnalysisStore = service(c, 'linkedinAnalysisStore');
    const history = await linkedinAnalysisStore.getLinkedInAnalysisHistory(c.get('user').id, getQuery(c).limit);
    return c.json({ history });
  } catch (err) {
    console.error('LinkedIn history error:', err);
    return c.json({ error: 'Failed to fetch LinkedIn history' }, 500);
  }
});

router.get('/linkedin/history/:id', authenticateToken, async (c) => {
  try {
    const linkedinAnalysisStore = service(c, 'linkedinAnalysisStore');
    const analysis = await linkedinAnalysisStore.getLinkedInAnalysisById(c.get('user').id, c.req.param('id'));

    if (!analysis) {
      return c.json({ error: 'LinkedIn analysis not found' }, 404);
    }
    ...
    return c.json({ error: 'Failed to fetch LinkedIn analysis' }, 500);
  }
});
...
router.post('/linkedin/generate-headline', authenticateToken, requirePlan(2), async (c) => {
  try {
    const linkedinOptimizerService = service(c, 'linkedinOptimizerService');
    const { roleInfo, companyContext, achievements, targetRoles } = getBody(c);
    if (!roleInfo) {
      return c.json({ error: 'roleInfo is required' }, 400);
    }
    const result = await linkedinOptimizerService.generateHeadline(roleInfo, companyContext, achievements, targetRoles);
    return c.json(result);
  } catch (err) {
    console.error('Headline generation error:', err);
    return c.json({ error: 'Failed to generate headline options' }, 500);
  }
});

router.post('/linkedin/generate-about', authenticateToken, requirePlan(2), async (c) => {
  try {
    const linkedinOptimizerService = service(c, 'linkedinOptimizerService');
    const { profileContext, skills, achievements, targetRoles, targetIndustries } = getBody(c);
    if (!profileContext) {
      return c.json({ error: 'profileContext is required' }, 400);
    }
    const result = await linkedinOptimizerService.generateAbout(profileContext, skills, achievements, targetRoles, targetIndustries);
    return c.json(result);
  } catch (err) {
    console.error('About generation error:', err);
    return c.json({ error: 'Failed to generate about section' }, 500);
  }
});

router.post('/linkedin/generate-experience', authenticateToken, requirePlan(2), async (c) => {
  try {
    const linkedinOptimizerService = service(c, 'linkedinOptimizerService');
    const { jobTitle, company, responsibilities, achievements } = getBody(c);
    if (!jobTitle || !company) {
      return c.json({ error: 'jobTitle and company are required' }, 400);
    }
    const result = await linkedinOptimizerService.generateExperienceDescription(jobTitle, company, responsibilities, achievements);
    return c.json(result);
  } catch (err) {
    console.error('Experience generation error:', err);
    return c.json({ error: 'Failed to generate experience description' }, 500);
  }
});
...
router.post('/jobmatch', authenticateToken, (c) => {
  const { jobDescription, userSkills } = getBody(c);
  if (!jobDescription || !userSkills) {
    return c.json({ error: 'Job description and user skills are required' }, 400);
  }
...
// Not part of the router API: exposed non-enumerably so tests can fuzz the decoder against Buffer directly.
Object.defineProperty(router, 'decodeBase64Utf8', { value: decodeBase64Utf8 });
