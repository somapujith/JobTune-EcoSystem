...
/**
 * Worker port of backend/src/routes/studyTools.js (mounted at /api/study-tools).
 * Endpoints (all authenticateToken + requirePlan(1)): POST /notes/generate, POST /flashcards/generate,
 * POST /quiz/generate, POST /quiz/submit.
 *
 * Changes vs Express (platform-forced only): callAI/extractJSON -> injected aiClient service;
 * studyHistoryService (owned by the leaf2 slice) -> getServices(c).studyHistoryService, same API;
 * req/res -> Hono context.
 * Preserved as-is: the "non-blocking" history save is awaited inside its own try/catch and only warns on
 * failure (so a history outage never fails the request), Math.max(count, 5) string coercion, NaN score for
 * an empty questions array, all fallback content, all messages and status codes.
 */
...
/**
 * The history service lives in another slice's registry. If it is not registered, fail loudly (masked 500)
 * at the top of the request instead of letting a TypeError inside the "non-blocking" save's try/catch
 * silently discard every study session. (In Express the missing module would have failed at boot.)
 */
function getStudyHistoryService(c) {
  const service = getServices(c).studyHistoryService;
  if (!service || typeof service.saveSession !== 'function') {
    throw new Error('studyHistoryService is not registered in the service container');
  }
  return service;
}
...
router.post('/notes/generate', authenticateToken, requirePlan(1), async (c) => {
  const { aiClient } = getServices(c);
  const studyHistoryService = getStudyHistoryService(c);
  try {
    const { topic, type = 'detailed' } = getBody(c);

    if (!topic || topic.trim().length < 3) {
      return c.json({ error: 'Topic must be at least 3 characters' }, 400);
    }
    ...
    try {
      await studyHistoryService.saveSession(c.get('user').id, {
        sessionType: 'notes',
        topic,
        difficulty: null,
        score: null,
...
router.post('/flashcards/generate', authenticateToken, requirePlan(1), async (c) => {
  const { aiClient } = getServices(c);
  const studyHistoryService = getStudyHistoryService(c);
  try {
    const { topic, count = 10 } = getBody(c);
...
router.post('/quiz/generate', authenticateToken, requirePlan(1), async (c) => {
  const { aiClient } = getServices(c);
  try {
    const { topic, difficulty = 'medium', count = 10 } = getBody(c);
    ...
    return c.json({ success: true, data: { quizId, topic, difficulty, questions } });
  } catch (err) {
...
router.post('/quiz/submit', authenticateToken, requirePlan(1), async (c) => {
  const studyHistoryService = getStudyHistoryService(c);
  try {
    const { quizId, answers, questions, topic, difficulty, timeTaken } = getBody(c);

    if (!quizId || !answers || !questions) {
      return c.json({ error: 'quizId, answers, and questions are required' }, 400);
    }
