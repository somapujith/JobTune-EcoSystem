...
/**
 * Worker port of backend/src/routes/community.js (mounted at /api/community).
 * Wave 3F, slice "large1". Ported from git HEAD 38d8130a (the Express file has no uncommitted edits).
 *
 ...
 *
 * Every route keeps the Express middleware chain: authenticateToken -> requirePlan(n), same n.
 * Handler bodies, SQL text, validation, messages, status codes and fallbacks are unchanged.
 *
 * Deviations forced by the platform (see docs/migration/wave/large1.md):
 *   - the module-load bootstrap that created the five community tables is NOT ported (ADR 6.5);
 *     the tables are assumed to exist already (Render created them).
 *   - AI goes through the injected aiClient service (getServices(c).aiClient).
 *   - request body / query / path params come from getBody(c) / getQuery(c) / c.req.param(...).
 *
 * PRESERVED PRE-EXISTING BUG (do not fix in the port, ADR 4.3): the two /communication/* handlers
 * pass callAI's whole result object ({ ok, data, error }) to extractJSON instead of its .data
 * string, so extractJSON throws and the handlers ALWAYS answer with the canned fallback feedback,
 * never the AI's. This port keeps that exact call shape.
 */
...
router.get('/forums', authenticateToken, requirePlan(1), async (c) => {
  try {
...
router.get('/forums/:category', authenticateToken, requirePlan(1), async (c) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      return c.json({ threads: result.rows, page, hasMore: result.rows.length === limit });
    }
    const filtered = FALLBACK_THREADS.filter(t => t.category === category);
    return c.json({ threads: filtered, page: 1, hasMore: false });
  } catch (err) {
...
router.get('/forums/thread/:id', authenticateToken, requirePlan(1), async (c) => {
  const id = c.req.param('id');
  try {
    const db = getDb(c);
    const threadResult = await db.query('SELECT * FROM community_threads WHERE id = $1', [id]);
    if (threadResult.rows.length === 0) {
      // Return fallback thread
      const fallback = FALLBACK_THREADS.find(t => t.id === parseInt(id));
      if (fallback) return c.json({ thread: fallback, replies: [] });
      return c.json({ error: 'Thread not found' }, 404);
    }

    // Increment view count
    await db.query('UPDATE community_threads SET views = views + 1 WHERE id = $1', [id]);

    const repliesResult = await db.query(
      'SELECT * FROM community_replies WHERE thread_id = $1 ORDER BY created_at ASC',
      [id]
    );
    return c.json({ thread: threadResult.rows[0], replies: repliesResult.rows });
  } catch (err) {
    console.error('GET /forums/thread/:id error:', err.message);
    const fallback = FALLBACK_THREADS.find(t => t.id === parseInt(id));
    return c.json({ thread: fallback || FALLBACK_THREADS[0], replies: [] });
  }
});

// POST /forums/thread — create new thread
router.post('/forums/thread', authenticateToken, requirePlan(1), async (c) => {
  const { title, body, category, tags } = getBody(c);
  ...
  try {
    ...
    const result = await db.query(
      ...
      [userId, authorName, title.slice(0, 500), body.slice(0, 5000), safeCategory, JSON.stringify(safeTags)]
    );
    return c.json({ thread: result.rows[0] });
  } catch (err) {
...
router.post('/forums/thread/:id/reply', authenticateToken, requirePlan(1), async (c) => {
  const id = c.req.param('id');
  const { body } = getBody(c);
  if (!body || typeof body !== 'string') return c.json({ error: 'Reply body is required' }, 400);
  ...
  try {
    ...
    const result = await db.query(
      ...
      [id, userId, authorName, body.slice(0, 5000)]
    );

    // Update reply count
    await db.query('UPDATE community_threads SET reply_count = reply_count + 1 WHERE id = $1', [id]);

    return c.json({ reply: result.rows[0] });
  } catch (err) {
...
router.post('/forums/thread/:id/vote', authenticateToken, requirePlan(1), async (c) => {
  const id = c.req.param('id');
  const { direction } = getBody(c);
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      if (existing.rows[0].direction === direction) {
        return c.json({ message: 'Already voted' });
      }
      // Change vote direction
      await db.query('UPDATE community_votes SET direction = $1 WHERE id = $2', [direction, existing.rows[0].id]);
      const delta = direction === 'up' ? 2 : -2;
      await db.query('UPDATE community_threads SET votes = votes + $1 WHERE id = $2', [delta, id]);
    } else {
      await db.query(
    ...
    return c.json({ success: true }); // Don't fail the UI
  }
});
...
router.get('/groups', authenticateToken, requirePlan(1), async (c) => {
  try {
    const result = await getDb(c).query('SELECT * FROM study_groups ORDER BY created_at DESC LIMIT 20');
    if (result.rows.length > 0) {
      return c.json({ groups: result.rows });
    }
    return c.json({ groups: FALLBACK_GROUPS });
  } catch (err) {
...
router.post('/groups', authenticateToken, requirePlan(1), async (c) => {
  const { name, topic, description, maxMembers } = getBody(c);
  if (!name || !topic) {
    return c.json({ error: 'Name and topic are required' }, 400);
  }
  ...
  try {
    const result = await getDb(c).query(
      `INSERT INTO study_groups (creator_id, name, topic, description, max_members, color)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [c.get('user').id, name.slice(0, 255), topic.slice(0, 255), (description || '').slice(0, 2000), maxMembers || 30, color]
    );
    return c.json({ group: result.rows[0] });
  } catch (err) {
...
router.get('/events', authenticateToken, requirePlan(1), async (c) => {
  try {
    const result = await getDb(c).query(
    ...
    return c.json({ events: FALLBACK_EVENTS });
  } catch (err) {
...
router.get('/leaderboard', authenticateToken, requirePlan(1), async (c) => {
  // For now, return curated fallback data.
  // In production, aggregate from user activity tables.
  return c.json({ leaderboard: FALLBACK_LEADERBOARD });
});
...
router.post('/communication/practice', authenticateToken, requirePlan(2), async (c) => {
  const { type, content, scenario } = getBody(c);
  ...
  if (type === 'hr-communication') {
    try {
      const raw = await aiClient.callAI({
      ...
      const feedback = aiClient.extractJSON(raw);
      ...
      return c.json({ feedback: generateFallbackHRFeedback(trimmedContent, scenario) });
    }
  }
  ...
  try {
    ...
    return c.json({ feedback: generateFallbackPracticeFeedback(trimmedContent, scenario) });
  }
});

// POST /communication/email — analyze email
router.post('/communication/email', authenticateToken, requirePlan(2), async (c) => {
  const { email, subject, template } = getBody(c);
  if (!email || !email.trim()) {
    return c.json({ error: 'Email content is required' }, 400);
  }
  ...
  try {
    ...
    const analysis = aiClient.extractJSON(raw);
    ...
    return c.json({ analysis: generateFallbackEmailFeedback(trimmedEmail, subject, template) });
  }
});
