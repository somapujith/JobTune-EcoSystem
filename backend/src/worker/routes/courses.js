...
/**
 * Worker port of backend/src/routes/courses.js (mounted at /api/courses).
 * Wave 3F, slice "large1". Ported from git HEAD 38d8130a (the Express file has no uncommitted edits).
 *
 * Every route keeps the Express middleware chain: authenticateToken -> requirePlan(1).
 * Handler bodies, SQL text, validation, messages, status codes and fallbacks are unchanged.
 * Route order is behaviour and is kept: /progress, /paths and /paths/:id are registered before /:id.
 *
 * Deviations forced by the platform (see docs/migration/wave/large1.md):
 *   - the module-load bootstrap that created the courses / enrollments / learning_paths / streaks
 *     tables is NOT ported (ADR 6.5); the tables are assumed to exist already (Render created them).
 *     Reads already fall back to the built-in data when a table is missing, as on Express.
 *   - the Express handlers wrapped their body in try/catch and called next(err); here an exception
 *     simply propagates to app.onError, which renders the identical masked 500 (README cheat sheet).
 *   - the handler's Hono context parameter is named `ctx` (not `c`) because these handlers use `c`
 *     as the loop variable for a course in many inner arrow functions.
 *   - request query / body / path params come from getQuery(ctx) / getBody(ctx) / ctx.req.param(...).
 */
...
async function getAllPaths(db) {
  try {
    const { rows } = await db.query('SELECT * FROM learning_paths ORDER BY id');
    if (rows.length > 0) return rows;
  } catch { /* fall through */ }
...
router.get('/', authenticateToken, requirePlan(1), async (ctx) => {
  const { category, difficulty, search, page = 1 } = getQuery(ctx);
  const perPage = 12;

  let courses = await getAllCourses(getDb(ctx));
...
router.get('/progress', authenticateToken, requirePlan(1), async (ctx) => {
  const db = getDb(ctx);
...
router.get('/paths', authenticateToken, requirePlan(1), async (ctx) => {
  const db = getDb(ctx);
...
router.get('/paths/:id', authenticateToken, requirePlan(1), async (ctx) => {
  const db = getDb(ctx);
  const pathId = Number(ctx.req.param('id'));
  const paths = await getAllPaths(db);
  const path = paths.find(p => p.id === pathId);

  if (!path) return ctx.json({ error: 'Learning path not found' }, 404);
...
router.get('/:id', authenticateToken, requirePlan(1), async (ctx) => {
  const db = getDb(ctx);
  const courseId = Number(ctx.req.param('id'));
  const courses = await getAllCourses(db);
  const course = courses.find(c => c.id === courseId);

  if (!course) return ctx.json({ error: 'Course not found' }, 404);

  let enrollment = null;
  try {
    const { rows } = await db.query(
      'SELECT * FROM course_enrollments WHERE user_id = $1 AND course_id = $2',
      [ctx.get('user').id, courseId]
    );
...
router.post('/:id/enroll', authenticateToken, requirePlan(1), async (ctx) => {
  const db = getDb(ctx);
  const courseId = Number(ctx.req.param('id'));
  const courses = await getAllCourses(db);
  const course = courses.find(c => c.id === courseId);
  ...
  return ctx.json({ success: true, message: `Enrolled in "${course.title}"` });
});

// ── POST /:id/progress — Update course progress ─────────────────────────────
router.post('/:id/progress', authenticateToken, requirePlan(1), async (ctx) => {
  const db = getDb(ctx);
  const userId = ctx.get('user').id;
  const courseId = Number(ctx.req.param('id'));
  const { lessonId, completed } = getBody(ctx);

  if (!lessonId) return ctx.json({ error: 'lessonId is required' }, 400);
  ...
  await db.query(
    ...
    [progress, JSON.stringify(completedLessons), userId, courseId]
  );

  // Update streak
  const today = new Date().toISOString().split('T')[0];
  try {
    const { rows } = await db.query('SELECT * FROM learning_streaks WHERE user_id = $1', [userId]);
    if (rows.length > 0) {
      const streak = rows[0];
  ...
  return ctx.json({ success: true, data: { progress, completedLessons } });
});
