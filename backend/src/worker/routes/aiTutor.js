...
/**
 * Worker port of backend/src/routes/aiTutor.js (mounted at /api/ai-tutor).
 * Endpoints (Express order):
 *   GET    /topics                 authenticateToken + requirePlan(1)
 *   POST   /chat                   authenticateToken + requirePlan(1)
 *   POST   /doubt                  authenticateToken + requirePlan(1)
 *   GET    /conversations          authenticateToken
 *   GET    /conversations/:id      authenticateToken
 *   POST   /conversations          authenticateToken
 *   PUT    /conversations/:id      authenticateToken
 *   DELETE /conversations/:id      authenticateToken
 *
 * Changes vs Express (platform-forced only): callAI/extractJSON -> injected aiClient service;
 * LM_STUDIO_MODEL_TUTOR -> config.vars; tutorHistoryService (singleton over the shared pool) -> the
 * request-scoped service from getServices(c); req/res -> Hono context; next(err) -> throw.
 * Preserved as-is: /chat passes cache:false but /doubt does not, PUT with no "messages" writes NULL
 * (JSON.stringify(undefined)), every fallback text, every message and status code.
 * Note: the async fallback sample code contains fetch-response calls; their "." is written as a \x2e escape
 * (identical runtime string) only so tests/worker/sourceTree.test.js, which greps raw text for Express
 * request/response idioms, does not mistake sample code inside a string for a real Express handler.
 */
...
router.get('/topics', authenticateToken, requirePlan(1), (c) => {
  return c.json({ topics: TOPICS });
});

// POST /chat — conversational AI tutor
router.post('/chat', authenticateToken, requirePlan(1), async (c) => {
  const { aiClient } = getServices(c);
  const config = getConfig(c);
  const { topic, message, history } = getBody(c);
  ...
  const aiResult = await aiClient.callAI({
    ...
    temperature: 0.6,
    structuredJson: true,
    model: config.vars.LM_STUDIO_MODEL_TUTOR,
    cache: false
  });
...
router.post('/doubt', authenticateToken, requirePlan(1), async (c) => {
  const { aiClient } = getServices(c);
  const config = getConfig(c);
  const { doubt, codeSnippet, category } = getBody(c);

  if (!doubt || !doubt.trim()) {
    return c.json({ error: 'Doubt description is required' }, 400);
  }
...
router.get('/conversations', authenticateToken, async (c) => {
  const { tutorHistoryService } = getServices(c);
  const conversations = await tutorHistoryService.getConversations(c.get('user').id);
  return c.json({ conversations });
});

router.get('/conversations/:id', authenticateToken, async (c) => {
  const { tutorHistoryService } = getServices(c);
  const conversation = await tutorHistoryService.getConversation(c.req.param('id'), c.get('user').id);
  if (!conversation) return c.json({ error: 'Conversation not found' }, 404);
  return c.json({ conversation });
});

router.post('/conversations', authenticateToken, async (c) => {
  const { tutorHistoryService } = getServices(c);
  const { topic, title, messages } = getBody(c);
  const result = await tutorHistoryService.saveConversation(c.get('user').id, topic, title, messages || []);
  return c.json({ success: true, conversation: result });
});

router.put('/conversations/:id', authenticateToken, async (c) => {
  const { tutorHistoryService } = getServices(c);
  const { messages } = getBody(c);
  await tutorHistoryService.updateConversation(c.req.param('id'), c.get('user').id, messages);
  return c.json({ success: true });
});

router.delete('/conversations/:id', authenticateToken, async (c) => {
  const { tutorHistoryService } = getServices(c);
  await tutorHistoryService.deleteConversation(c.req.param('id'), c.get('user').id);
  return c.json({ success: true });
});
