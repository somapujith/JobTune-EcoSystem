...
/**
 * /api/resume (part 1 of 2): Worker port of backend/src/routes/resume.js (HEAD).  (docs slice)
 * Mounted BEFORE routes/resumeV2.js on the same prefix, like Express (see routes/mounts/docs.js).
 *
 * 8 endpoints, all `authenticateToken` only (no requirePlan), in the Express registration order:
 *   POST   /upload     multipart `resume` (10 MB; pdf, docx, msword)   analyse + persist a resume
 *   GET    /list
 *   GET    /scores
 *   GET    /:id
 *   DELETE /:id
 *   POST   /tune       multipart `resume` + text fields jobDescription / outputFormat
 *   POST   /ai-edit    AI edit through the injected aiClient
 *   POST   /build      questionnaire -> PDF / DOCX (base64 in the JSON body)
 *
 * multer -> readMultipart (services/docs/multipart.js, a wrapper over lib/upload.js readUpload): same 10 MB cap
 * and mimetype allowlist, plus multer's text fields (the body object of a multipart request). Upload errors
 * (wrong type, too large, unexpected field) have no `status`, so onError renders them as the same masked 500
 * Express produced. `pdf-parse` -> services/docs/pdfParse.js (pinned pdf.js v1.10.100, Uint8Array input).
 * pdfkit / docx are loaded lazily (services/docs/docLibs.js).
 *
 * Not ported, on purpose: the module-load `CREATE TABLE IF NOT EXISTS resumes` + `ALTER TABLE ... ADD COLUMN`
 * bootstrap of the Express file (ADR 6.5). The table and every column it adds already exist in Neon; a
 * missing column would surface as a 500 from the INSERT, exactly where Render would have healed itself.
 *
 * Preserved behaviours worth knowing (ADR 4.3, not fixed):
 *   - `extractText` returns '' for DOCX / msword (there is no DOCX branch), so POST /tune on a DOCX answers 422
 *     and POST /upload on a DOCX scores an empty string.
 *   - a corrupt PDF falls back to the raw bytes decoded as UTF-8 and is scored as text.
 *   - `atsScore` is clamped to a floor of 90 (Math.max(90, ...)), so it never reports below 90.
 *   - /:id is registered before /tune etc. but only for GET/DELETE, so there is no shadowing.
 */
...
const { parsePdf, decodeUtf8, toBase64 } = require('../services/docs/docText');
const docLibs = require('../services/docs/docLibs');

const router = createRouter();

// multer({ limits: { fileSize: 10MB }, fileFilter: pdf | docx | msword })
const UPLOAD = {
  field: 'resume',
...
function scoreResume(text, sections, fileName) { // eslint-disable-line no-unused-vars
  const upper = text.toUpperCase();
...
// multer left the text fields of a multipart request in the body; anything else keeps the parsed JSON/urlencoded body
function requestBody(c, fields) {
  return fields !== undefined ? fields : getBody(c);
}

// POST /api/resume/upload — analyze a resume file
router.post('/upload', authenticateToken, async (c) => {
  const { file, fields } = await readMultipart(c, UPLOAD);
  const body = requestBody(c, fields);
  const fileName = file ? file.originalname : (body && body.fileName) || 'resume.pdf';
  ...
  const rows = await db.query('SELECT * FROM resumes WHERE id = $1', [result.rows[0].id]);
...
router.get('/list', authenticateToken, async (c) => {
  ...
  return c.json({ success: true, data: rows.rows.map(parseJsonFields) });
});

// GET /api/resume/scores — latest scores for the logged-in user
router.get('/scores', authenticateToken, async (c) => {
  const rows = await getDb(c).query(
    `SELECT scores, overall_score FROM resumes WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1`,
    [c.get('user').id]
  );
  if (rows.rows.length === 0) return c.json({ success: true, data: null });
...
router.get('/:id', authenticateToken, async (c) => {
  ...
  const chunksRow = await db.query(
...
router.delete('/:id', authenticateToken, async (c) => {
  ...
  if (result.rowCount === 0) {
    return c.json({ error: 'Resume not found or you do not have permission to delete it.' }, 404);
  }
...
router.post('/tune', authenticateToken, async (c) => {
  const { file, fields } = await readMultipart(c, UPLOAD);
  const body = requestBody(c, fields);
  const jobDescription = (body && body.jobDescription) || '';
  const outputFormat = (body && body.outputFormat) || 'docx';

  if (!file) return c.json({ error: 'Resume file is required.' }, 400);
  if (!jobDescription.trim()) return c.json({ error: 'Job description is required.' }, 400);

  // Extract resume text (reuse existing util)
  const resumeText = await extractText(file);
  if (!resumeText.trim()) {
    return c.json({ error: 'Could not extract text from this file. Please use a text-based PDF.' }, 422);
  }
...
router.post('/ai-edit', authenticateToken, async (c) => {
  const { instruction, resumeText, context } = getBody(c);
...
- If the resume is thin, suggest concrete ways to add substance (projects, certifications, metrics)`;
  ...
  const aiResult = await aiClient.callAI({
    ...
    maxTokens: 512,
    temperature: 0.2,
    model: getConfig(c).vars.LM_STUDIO_MODEL_RESUME,
  });

  if (aiResult.ok && aiResult.data) {
    return c.json({ success: true, data: { suggestion: aiResult.data } });
  }
...
async function markdownToDocxBuffer(markdownText) {
  const { Document, Paragraph, TextRun, HeadingLevel, Packer } = await docLibs.loadDocx();
...
router.post('/build', authenticateToken, async (c) => {
  const {
    fullName = '',
    email = '',
    phone = '',
  ...
  if (!targetJobDescription.trim()) {
    return c.json({ error: 'Target Job Description is required.' }, 400);
  }
  if (!fullName.trim()) {
