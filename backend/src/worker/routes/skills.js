...
/**
 * Worker port of backend/src/routes/skills.js (mounted at /api/skills).
 * Endpoints: GET /questions, POST /assessment, GET /history (all authenticateToken + requirePlan(1)).
 *
 * Changes vs Express (platform-forced only): pool -> getDb(c); callAI/extractJSON -> the injected
 * aiClient service; the LM_STUDIO_MODEL_SKILLS env var -> config.vars; req/res -> Hono context;
 * next(err) -> throw (onError masks 5xx exactly like errorHandler).
 * Preserved as-is: the unused `count` in /questions (the bank always yields 4+2+2), the JSONB
 * double-encode, the raw-answers fallback, "SELECT *" echo, every message and status code.
 */
...
router.get('/questions', authenticateToken, requirePlan(1), async (c) => {
  try {
    const difficulty = getQuery(c).difficulty || 'medium';
    const count = Math.min(parseInt(getQuery(c).count) || 8, 15);
    const questions = selectQuestions(difficulty, count);
    return c.json({ questions, total: questions.length, difficulty });
  } catch (err) {
...
router.post('/assessment', authenticateToken, requirePlan(1), async (c) => {
  const { aiClient } = getServices(c);
  const config = getConfig(c);
  const db = getDb(c);
  const { answers } = getBody(c);
  ...
  const aiResult = await aiClient.callAI({
    ...
    maxTokens: 1200,
    model: config.vars.LM_STUDIO_MODEL_SKILLS
  });
  ...
  const rows = await db.query('SELECT * FROM skill_assessments WHERE id = $1', [dbResult.rows[0].id]);
