...
/**
 * Worker port of backend/src/routes/projectBuilder.js (mounted at /api/project-builder).
 ...
 *   - the module-load workspace-table bootstrap of the Express file is NOT ported (ADR 6.5);
 *     the table is assumed to exist already (Render created it).
 *   - AI goes through the injected aiClient service (getServices(c).aiClient): same callAI /
 *     extractJSON signatures; the per-feature model comes from config.vars, read per request.
 *   - request body / path params come from getBody(c) / c.req.param(...).
 */
...
// Express read these two variables at request time inside each handler:
// the project model, falling back to the default model. Now taken from the injected config.
function projectModel(c) {
  const { vars } = getConfig(c);
  return vars.LM_STUDIO_MODEL_PROJECT || vars.LM_STUDIO_MODEL;
}

// ── POST /generate — Generate project plan from description ───────────────────
router.post('/generate', authenticateToken, requirePlan(1), async (c) => {
  try {
    const { description, difficulty, template } = getBody(c);

    if (!description && !template) {
      return c.json({ error: 'Project description or template is required.' }, 400);
    }
...
JSON schema:
{
  ...
  "implementationSteps": [{"step":number,"title":"string","description":"string","tasks":["string"]}]
    ...
    if (aiResult.ok) {
      plan = aiClient.extractJSON(aiResult.data);
    ...
    return c.json({ error: 'Failed to generate project plan.' }, 500);
  }
});

// ── POST /readme — Generate README from project plan ──────────────────────────
router.post('/readme', authenticateToken, requirePlan(1), async (c) => {
  try {
    const { projectPlan } = getBody(c);
    if (!projectPlan) {
      return c.json({ error: 'Project plan is required.' }, 400);
    }
...
Folder Structure: ${JSON.stringify(projectPlan.folderStructure)}`;
    ...
    return c.json({ readme, aiPowered });
  } catch (err) {
...
router.post('/deployment', authenticateToken, requirePlan(1), async (c) => {
  try {
    const { projectPlan, platform } = getBody(c);
    if (!projectPlan) {
      return c.json({ error: 'Project plan is required.' }, 400);
    }
...
JSON schema:
{
  ...
  "notes": ["string"]
    ...
    if (aiResult.ok) {
      guide = aiClient.extractJSON(aiResult.data);
    ...
    return c.json({ guide, aiPowered });
  } catch (err) {
    console.error('Deployment guide generation error:', err.message);
    return c.json({ error: 'Failed to generate deployment guide.' }, 500);
  }
});

// ── GET /workspace — Get user's saved projects ───────────────────────────────
router.get('/workspace', authenticateToken, requirePlan(1), async (c) => {
  try {
...
router.post('/workspace', authenticateToken, requirePlan(1), async (c) => {
  try {
    const { name, plan, status, tasks, notes } = getBody(c);
    if (!name) {
...
router.put('/workspace/:id', authenticateToken, requirePlan(1), async (c) => {
  try {
    const id = c.req.param('id');
    const { status, tasks, notes } = getBody(c);
...
router.get('/templates', authenticateToken, requirePlan(1), (c) => {
  return c.json({ templates: PROJECT_TEMPLATES });
});
